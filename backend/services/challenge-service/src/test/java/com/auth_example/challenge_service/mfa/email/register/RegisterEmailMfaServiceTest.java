...
public class RegisterEmailMfaServiceTest {
    @Mock
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: getType] - should return MfaChallengeType.EMAIL")
    public void shouldReturnEmailMfaType() {
        // arrange
        // act
        MfaChallengeType type = registerEmailMfaService.getType();
        // assert
        assertEquals(MfaChallengeType.EMAIL, type);
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: create] - should return challenge object if challenge is created and stored in redis successfully")
    public void shouldReturnChallengeIfChallengeSuccessfullyCreated() {
        ...
        // act
        EmailMfaChallenge functionCall = registerEmailMfaService.create(mockRequest);
        // assert
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: create] - should throw IllegalArgumentException if otp service cannot generate otp successfully")
    public void shouldNotReturnIfCreateEmailMfaIllegalArgumentException() {
        ...
        // act
        // assert
        assertThrows(IllegalArgumentException.class, () -> registerEmailMfaService.create(mockRequest));
        verify(mapper, never()).createChallengeRequestToMfaChallenge(any(), anyString());
        verify(redisService, never()).storeMfaEntry(any(), any());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: create] - should throw RedisException if redis service fails to execute store mfa entry")
    public void shouldNotReturnIfCreateEmailMfaRedisException() {
        ...
        // act
        // assert
        assertThrows(RedisException.class, () -> registerEmailMfaService.create(mockRequest));
        verify(otpService).generateOtp(anyInt());
        verify(mapper).createChallengeRequestToMfaChallenge(any(), anyString());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: validate] - should return challenge if validation is successful")
    public void shouldReturnChallengeIfChallengeSuccessfullyValidated() {
        ...
        // act
        EmailMfaChallenge functionCall = registerEmailMfaService.validate(mockRequest);
        // assert
        assertEquals(mockChallenge, functionCall);
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: validate] - should throw [ChallengeNotFoundException] if no challenge found in redis")
    public void shouldThrowChallengeNotFoundExceptionIfNoChallengeFoundOnRedis() {
        ...
        // act
        // assert
        assertThrows(ChallengeNotFoundException.class, () -> registerEmailMfaService.validate(mockRequest));
        verify(redisService).findMfaById(mockRequest.challengeId());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: validate] - should throw [EmailMismatchException] if email from redis-stored challenge does not match with the requester")
    public void shouldThrowEmailMismatchExceptionIfEmailDoesNotMatch() {
        ...
        // act
        // assert
        assertThrows(EmailMismatchException.class, () -> registerEmailMfaService.validate(mockRequest));
        verify(redisService).findMfaById(mockRequest.challengeId());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: validate] - should throw [CodeDoesNotMatchException] if validation code does not match")
    public void shouldThrowCodeDoesNotMatchExceptionIfCodeDoesNotMatch() {
        ...
        // act
        // assert
        assertThrows(CodeDoesNotMatchException.class, () -> registerEmailMfaService.validate(mockRequest));
        verify(redisService).findMfaById(mockRequest.challengeId());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: findOneById] - should return challenge if challenge id exist in redis")
    public void shouldReturnChallengeIfIdExistInRedis() {
        ...
        // act
        EmailMfaChallenge functionCall = registerEmailMfaService.findOneById(mockUuid);
        // assert
        assertEquals(mockChallenge, functionCall);
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: findOneById] - should not return anything if challenge id does not exist in redis")
    public void shouldNotReturnIfIdDoesNotExistInRedis() {
        ...
        // act
        // assert
        assertThrows(ChallengeNotFoundException.class, () -> registerEmailMfaService.findOneById(mockUuid));
        verify(redisService).findMfaById(mockUuid);
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: findOneOrThrow] - should return challenge if email exist in redis")
    public void shouldReturnChallengeIfEmailExistInRedis() {
        ...
        // act
        EmailMfaChallenge functionCall = registerEmailMfaService.findOneOrThrow(mockRequest);
        // assert
        assertEquals(mockChallenge, functionCall);
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: findOneOrThrow] - should throw [ChallengeNotFoundException] if email exist in redis")
    public void shouldThrowChallengeNotFoundExceptionIfEmailDoesNotExistInRedis() {
        ...
        // act
        // assert
        assertThrows(ChallengeNotFoundException.class, () -> registerEmailMfaService.findOneOrThrow(mockRequest));
        verify(redisService).findMfaByIdentity(mockRequest.email());
    }

    @Test
    @DisplayName("[RegisterEmailMfaService :: findOne] - should return challenge if email exist in redis")
    public void shouldReturnChallengeIfFindOneEmailExistInRedis() {
        // arrange
        EmailMfaFindByEmailRequest mockRequest = new EmailMfaFindByEmailRequest(mockChallenge.email());
        when(redisService.findMfaByIdentity(mockRequest.email())).thenReturn(Optional.ofNullable(mockChallenge));
        // act
        Optional<EmailMfaChallenge> functionCall = registerEmailMfaService.findOne(mockRequest);
        // assert
    ...
    @Test
    @DisplayName("[RegisterEmailMfaService :: findOne] - should not return anything if email does not exist in redis")
